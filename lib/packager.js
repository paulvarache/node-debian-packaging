...
Packager.prototype.makeArchive = function (name, dir) {
    return new Promise(function (resolve, reject) {
        var outputFile = path.join(this.TMP_DIR, name);
        var output = fs.createWriteStream(outputFile);
        output.on('close', function () {
            resolve(outputFile);
        });
        var dataArch = archiver.create('tar', { gzip: true });
        dataArch.on('error', reject);
        dataArch.pipe(output);
        dataArch.directory(dir, false);
        dataArch.finalize();
    }.bind(this));
};
...
Packager.prototype.ar = function (dest, files) {
    return new Promise(function (resolve, reject) {
        ...
        ar.on('exit', function (code) {
            if (code !== 0) {
                return reject(new Error('ar exited with code ' + code));
            }
            resolve();
        });
    });
